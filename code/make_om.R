make_om <- function(Fhist = "Fmsy", N1_state = "Fmsy",
  ...
  na <- 10
  basic_info$lengths <- seq(from = 1, to = 120, by = 2)
  ...
  ## age comps
  # basic_info$use_index_paa <- matrix(1, ncol = basic_info$n_fleets, nrow = ny)
  ...
  input = set_F_scenario(input, Fhist, Fmsy = 0.5, max_mult = max_mult_Fmsy, min_mult= min_mult_Fmsy,
