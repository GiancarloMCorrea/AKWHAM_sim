...
ggsave(filename = file.path(save_folder, paste0(paste(paa_gen_approach, 'par', sep = '-'), fig_type)), 
       plot = p3, width = img_width, height = 190, units = 'mm', dpi = img_res)
...
all_ecovs = c('stationary', 'trend')
for(j in seq_along(all_ecovs)) {
  this_ecov = all_ecovs[j]
  # Plot TS by variable:
...
for(j in seq_along(all_ecovs)) {
  this_ecov = all_ecovs[j]
  # Plot TS by variable:
  all_vars = as.character(1:max(temp$age))
  for(i in seq_along(all_vars)) {
