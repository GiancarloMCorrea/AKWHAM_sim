...
gf_mat_len = list(model = 'len-logistic', init_vals = mat_base_len)
gf_mat_age = list(model = 'age-logistic', init_vals = mat_base_age)

# Make OMs --------------------------------------------------------------
om_inputs = list()
# Structure of om_inputs:
for(i in 1:length(growth_scenarios)) {
  ...
  for(k in 1:length(paa_gen_scenarios)){
    
    om_inputs[[i]][[k]] = list()
    if(paa_gen_scenarios[k] == 'traditional') {
      gf_selectivity = gf_selectivity_age
      gf_mat = gf_mat_age
    }
    if(paa_gen_scenarios[k] == 'stepwise') {
      gf_selectivity = gf_selectivity_len
      gf_mat = gf_mat_len
    }
